...
/**
 * A dynamically configured factory, which constructs the Event instances 
 * using introspection to locate a static virtual constructor (or factory method) 
 * within one of a previously registered concrete event classes. 
 * 
 * Structurally this is a singleton, but it may not be reliably available 
 * via static reference from the web server context. 
 * 
 * Game-specific logic (often rules) must register all game-specific events before 
 * they will be needed. See "registerEvents" in PickupRules,  
 * 
 *   
 * @author Robert Ward
 *
...
public class EventUnmarshallers {
	...
	/**
	 * @param eventName a unique string that will be used as the 
	 *  key to lookup the desired event class
